...
Cursor suppresses its dialog with launch-time `--trust`, Muse suppresses its own with `--yolo`, and Devin skips its per-directory gate with `--respect-workspace-trust false`.
