...
devin is likewise verified for crewmate and scout launches ONLY, refused for a secondmate because no devin wake protocol exists; [`docs/verification/devin.md`](verification/devin.md) owns that evidence, including its per-task hooks in the worktree's uncommitted `.devin/config.local.json` and the delayed closing Escape its interrupt needs.
