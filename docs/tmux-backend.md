...
AGY is anchored to the exact `agy` identity for the same reason, so unrelated names containing that fragment remain ambiguous, and Devin is anchored to the exact `devin` identity, so names such as `devinfo` remain ambiguous.
